...
export default function PodcastsClient({ initialPodcasts }: { initialPodcasts: PodcastEvent[] }) {
  ...
  }, [initialPodcasts, filter, search])

  const featured = useMemo(() =>
    initialPodcasts.find(ep => ep.status === 'live') || initialPodcasts.find(ep => ep.status === 'upcoming'),
    [initialPodcasts]
  )
  const liveCount = useMemo(() => initialPodcasts.filter(ep => ep.status === 'live').length, [initialPodcasts])
  const upcomingCount = useMemo(() => initialPodcasts.filter(ep => ep.status === 'upcoming').length, [initialPodcasts])
  const totalEpisodes = initialPodcasts.length
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-2 flex-wrap">
            {(['all', 'upcoming', 'live', 'completed'] as const).map(s => {
              const cfg = s === 'all' ? { label: 'All Episodes', color: '#8B5CF6' }
                : s === 'live' ? { label: '🔴 Live', color: '#EF4444' }
                : s === 'upcoming' ? { label: 'Upcoming', color: '#3B82F6' }
