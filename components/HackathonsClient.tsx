...
export default function HackathonsClient({ initialHackathons }: { initialHackathons: HackathonEvent[] }) {
  ...
  }, [initialHackathons, filter, search])

  const featured = useMemo(() =>
    initialHackathons.find(h => h.status === 'upcoming' && h.prize_pool) || initialHackathons.find(h => h.status === 'upcoming'),
    [initialHackathons]
  )

  const totalPrize = useMemo(() =>
    initialHackathons.filter(h => h.status === 'upcoming').reduce((acc, h) => {
  ...
  const upcomingCount = useMemo(() => initialHackathons.filter(h => h.status === 'upcoming').length, [initialHackathons])
  const completedCount = useMemo(() => initialHackathons.filter(h => h.status === 'completed').length, [initialHackathons])
  ...
  return (
    <div className="min-h-screen pt-20" style={{ background: '#0a0f1e' }}>
      ...
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0a0f1e 0%, #1a1200 50%, #2d1500 100%)' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-20 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            {[
              { icon: '💻', n: initialHackathons.length, label: 'Total Hackathons' },
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <p className="text-white/20 text-xs mb-6">{filtered.length} hackathon{filtered.length !== 1 ? 's' : ''} found</p>
