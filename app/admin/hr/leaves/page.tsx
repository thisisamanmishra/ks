  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-navy font-heading">🏖️ Leave Requests</h1>
      ...
      <div className="flex gap-2 flex-wrap">
        {(['all', 'pending', 'approved', 'rejected'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 sm:px-4 py-2 rounded-full text-xs font-bold capitalize cursor-pointer transition-all ${filter === f ? 'bg-navy text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-navy/5'}`}>
              ...
              {filtered.map((req, i) => (
                <motion.div key={req.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-semibold text-navy text-sm">{req.user?.fullname || 'Unknown'}</p>
                      <p className="text-[10px] text-slate-400 capitalize">{req.user?.department || 'General'}</p>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold capitalize flex-shrink-0 ${STATUS_CLASSES[req.status] || 'bg-slate-100 text-slate-600'}`}>
                      {req.status}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      ...
                      <p className="font-semibold text-slate-600 capitalize">{req.leave_type}</p>
                    </div>
                    <div>
                      <p className="text-slate-400 text-[10px]">Days</p>
                      <p className="font-bold text-navy">{req.days}d</p>
                    </div>
                    <div>
                      <p className="text-slate-400 text-[10px]">Applied</p>
                      <p className="text-slate-500">{new Date(req.created_at).toLocaleDateString('en-IN')}</p>
                    </div>
                  </div>
                  ...
                  {req.reason && <p className="text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-2">{req.reason}</p>}
                  {req.status === 'pending' && (
                    <div className="flex gap-2">
                      <button onClick={() => handleAction(req.id, 'approved')} disabled={actionId === req.id}
                        className="flex-1 px-3 py-2.5 rounded-xl bg-green-50 text-green-600 text-xs font-bold hover:bg-green-100 cursor-pointer disabled:opacity-50 transition-colors">
                        ✓ Approve
                      </button>
                      <button onClick={() => handleAction(req.id, 'rejected')} disabled={actionId === req.id}
                        className="flex-1 px-3 py-2.5 rounded-xl bg-red-50 text-red-600 text-xs font-bold hover:bg-red-100 cursor-pointer disabled:opacity-50 transition-colors">
                        ✕ Reject
                      </button>
                    </div>
                  )}
                  {req.status !== 'pending' && req.reviewer && (
                    <p className="text-[10px] text-slate-400">Reviewed by {req.reviewer.fullname}</p>
                  )}
                </motion.div>
              ))}
            </div>
