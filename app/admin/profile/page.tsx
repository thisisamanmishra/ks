...
export default function MyProfilePage() {
  const [activeTab, setActiveTab] = useState<'docs'|'appraisals'|'leaves'>('docs')
  ...
  const uploadDoc = async () => {
    ...
    if (res.ok) { const d = await res.json(); setDocs(p => [d.doc, ...p]); setDocForm({ doc_type: 'joining_letter', title: '', doc_url: '', month: '', notes: '' }); setShowDocForm(false) }
  }
  ...
  const applyLeave = async () => {
    ...
    if (res.ok) { loadLeaves(); setLeaveForm({ leave_type: 'casual', start_date: '', end_date: '', days: '', reason: '' }); setShowLeaveForm(false) }
  }
  ...
  const calcStarRating = (rating: number) => {
    const full = Math.floor(rating)
    const half = rating % 1 >= 0.5
    return '★'.repeat(full) + (half ? '½' : '') + '☆'.repeat(5 - full - (half ? 1 : 0))
  }

  const filterDocType = (type: string) => docs.filter(d => d.doc_type === type)
  const payslips = filterDocType('payslip')
  ...
  const TABS = [
    { key: 'docs' as const, label: '📁 My Documents' },
    { key: 'appraisals' as const, label: '⭐ Appraisals' },
    { key: 'leaves' as const, label: '🏖️ Leave History' },
  ]
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      {activeTab === 'docs' && (
        <div className="space-y-5">
          <div className="flex justify-end">
            <button onClick={() => setShowDocForm(v => !v)} className="px-4 py-2.5 rounded-xl font-bold text-white text-sm cursor-pointer hover:opacity-90" style={{ background: '#1B3A6B' }}>
              + Upload Document
            </button>
          </div>
          ...
          <AnimatePresence>
            {showDocForm && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                <h4 className="font-bold text-navy mb-4">Upload Your Document</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <select value={docForm.doc_type} onChange={e => setDocForm(p => ({ ...p, doc_type: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none">
                    {Object.keys(DOC_TYPE_ICONS).map(t => <option key={t} value={t}>{DOC_TYPE_ICONS[t]} {t.replace('_',' ')}</option>)}
                  </select>
                  <input value={docForm.title} onChange={e => setDocForm(p => ({ ...p, title: e.target.value }))} placeholder="Document title *" className="col-span-2 px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />
                  <input value={docForm.doc_url} onChange={e => setDocForm(p => ({ ...p, doc_url: e.target.value }))} placeholder="Document URL / Google Drive link" className="col-span-2 px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />
                  {docForm.doc_type === 'payslip' && <input type="month" value={docForm.month} onChange={e => setDocForm(p => ({ ...p, month: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />}
                  <textarea value={docForm.notes} onChange={e => setDocForm(p => ({ ...p, notes: e.target.value }))} placeholder="Notes (optional)" rows={1} className="col-span-3 px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none resize-none" />
                </div>
                <div className="flex gap-3 mt-4">
                  <button onClick={uploadDoc} className="px-5 py-2.5 rounded-xl font-bold text-white text-sm cursor-pointer hover:opacity-90" style={{ background: '#1B3A6B' }}>Upload</button>
                  <button onClick={() => setShowDocForm(false)} className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm cursor-pointer">Cancel</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {payslips.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {payslips.map((d, i) => <DocCard key={d.id} doc={d} onDelete={deleteDoc} />)}
              </div>
            </div>
          )}
          ...
          {docs.length === 0 && !showDocForm && (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
              ...
              <button onClick={() => setShowDocForm(true)} className="px-4 py-2 rounded-xl font-bold text-white text-xs cursor-pointer hover:opacity-90" style={{ background: '#1B3A6B' }}>+ Upload Document</button>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'appraisals' && (
        <div className="space-y-4">
          {appraisals.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
              <span className="text-5xl block mb-3">⭐</span>
              <p className="text-slate-400 text-sm">No appraisals yet. Your manager/HR will add appraisal records here.</p>
            </div>
          ) : (
            appraisals.map((a, i) => (
              <motion.div key={a.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex items-start justify-between mb-4 flex-wrap gap-2">
                  <div>
                    <p className="font-bold text-navy text-lg">{a.period}</p>
                  ...
                  <div className="text-right">
                    {a.rating && (
                      <div>
                        <p className="text-amber-400 text-lg font-bold">{calcStarRating(a.rating)}</p>
                ...
                <div className="space-y-3">
                  {a.feedback && (
                    <div className="bg-blue-50 rounded-xl p-4">
                      <p className="text-xs font-bold text-blue-600 mb-1">💬 Manager Feedback</p>
                  ...
                  {a.areas_to_improve && (
                    <div className="bg-amber-50 rounded-xl p-4">
                      ...
                      <p className="text-sm text-slate-700">{a.areas_to_improve}</p>
                    </div>
                  )}
                </div>
              </motion.div>
      ...
      {activeTab === 'leaves' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowLeaveForm(v => !v)} className="px-4 py-2.5 rounded-xl font-bold text-white text-sm cursor-pointer hover:opacity-90" style={{ background: '#10B981' }}>
              + Apply for Leave
            </button>
          </div>
          ...
          <AnimatePresence>
            {showLeaveForm && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                <h4 className="font-bold text-navy mb-4">Apply for Leave</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <select value={leaveForm.leave_type} onChange={e => setLeaveForm(p => ({ ...p, leave_type: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none">
                    {LEAVE_TYPES.map(t => <option key={t} value={t}>{t.replace('_',' ')}</option>)}
                  </select>
                  <input type="date" value={leaveForm.start_date} onChange={e => setLeaveForm(p => ({ ...p, start_date: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />
                  <input type="date" value={leaveForm.end_date} onChange={e => setLeaveForm(p => ({ ...p, end_date: e.target.value }))} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />
                  <input type="number" min={1} value={leaveForm.days} onChange={e => setLeaveForm(p => ({ ...p, days: e.target.value }))} placeholder="Number of days" className="px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none" />
                  <textarea value={leaveForm.reason} onChange={e => setLeaveForm(p => ({ ...p, reason: e.target.value }))} placeholder="Reason for leave *" rows={1} className="col-span-2 px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none resize-none" />
                </div>
                <div className="flex gap-3 mt-4">
                  <button onClick={applyLeave} className="px-5 py-2.5 rounded-xl font-bold text-white text-sm cursor-pointer hover:opacity-90" style={{ background: '#10B981' }}>Submit Request</button>
                  <button onClick={() => setShowLeaveForm(false)} className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-sm cursor-pointer">Cancel</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Total Requests', count: leaves.length, color: '#3B82F6' },
              ...
              { label: 'Total Days', count: leaves.filter(l => l.status === 'approved').reduce((a, b) => a + b.days, 0), color: '#8B5CF6' },
            ].map(s => (
          ...
          {leaves.length === 0 ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <table className="w-full text-xs">
                <thead><tr className="bg-slate-50 border-b border-slate-100">{['Type','From','To','Days','Reason','Status','Applied On'].map(h => <th key={h} className="px-4 py-3 text-left font-bold text-slate-400 uppercase text-[10px]">{h}</th>)}</tr></thead>
                <tbody>
                  {leaves.map(l => {
                    const s = STATUS_STYLES[l.status] || { bg: '#F3F4F6', text: '#6B7280' }
                    return (
                      <tr key={l.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                        <td className="px-4 py-3 font-semibold text-navy capitalize">{l.leave_type.replace('_',' ')}</td>
                        ...
                        <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-[10px] font-bold capitalize" style={{ background: s.bg, color: s.text }}>{l.status}</span></td>
    ...
    <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-slate-400 capitalize">{doc.doc_type.replace('_',' ')}{doc.month ? ` · ${doc.month}` : ''}</p>
        </div>
      </div>
      {doc.notes && <p className="text-[10px] text-slate-400 mb-2 truncate">{doc.notes}</p>}
      <div className="flex gap-2">
        {doc.doc_url && <a href={doc.doc_url} target="_blank" rel="noreferrer" className="flex-1 text-center py-1 rounded-lg text-[10px] font-bold bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-blue-600 cursor-pointer transition-colors">🔗 Open</a>}
        <button onClick={() => onDelete(doc.id)} className="px-2 py-1 rounded-lg text-[10px] bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-500 cursor-pointer transition-colors">🗑</button>
      </div>
    </div>
  )
}
