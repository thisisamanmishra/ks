...
export default function EventsPage() {
  ...
  const featured = allEvents.find(e => e.status === 'upcoming' && e.prize_pool) || allEvents.find(e => e.status === 'upcoming')
  const typeCounts = TYPES.slice(1).map(t => ({ type: t, count: allEvents.filter(e => e.type === t && e.status === 'upcoming').length }))
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50 pt-20">
        ...
        <div className="max-w-7xl mx-auto px-4 py-10">
          ...
          {/* Featured event */}
          {!loading && featured && typeFilter === 'all' && statusFilter === 'upcoming' && !search && (
          ...
          {!loading && allEvents.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
              {[
                { label: 'Upcoming', n: allEvents.filter(e => e.status === 'upcoming').length, icon: '📅', c: '#3B82F6' },
                { label: 'Live Now', n: allEvents.filter(e => e.status === 'live').length, icon: '🔴', c: '#EF4444' },
                { label: 'Free Events', n: allEvents.filter(e => e.registration_fee === 0 && e.status === 'upcoming').length, icon: '🎁', c: '#10B981' },
                { label: 'Online', n: allEvents.filter(e => e.is_online && e.status === 'upcoming').length, icon: '🌐', c: '#8B5CF6' },
              ].map(s => (
