...
export async function sendEmail({ to, subject, html }: EmailOptions) {
  try {
    await transporter.sendMail({
      from: `"KaryaSaarthi" <${process.env.SMTP_USER || 'noreply@karyasaarthi.com'}>`,
      to,
      subject,
      html,
    })
...
const emailWrapper = (content: string) => `
  <div style="font-family: 'Inter', -apple-system, sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; padding: 20px;">
    <div style="background: linear-gradient(135deg, #1B3A6B, #0f2545); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
      <h1 style="color: white; margin: 0; font-size: 24px;">KaryaSaarthi</h1>
    </div>
    <div style="background: white; padding: 30px; border-radius: 0 0 12px 12px;">
      ${content}
    </div>
    <p style="text-align: center; color: #94a3b8; font-size: 12px; margin-top: 16px;">
      This is an automated email from KaryaSaarthi. Do not reply to this email.
    </p>
  </div>
`
...
export function otpEmail(otp: string, purpose: string, name?: string) {
  ...
  if (purpose === 'forgot_password') {
    return {
      subject: 'Password Reset OTP - KaryaSaarthi',
      html: emailWrapper(`
        ...
        <p style="color: #94a3b8; font-size: 12px;">⚠️ Never share this OTP with anyone. KaryaSaarthi will never ask for your OTP via phone or message.</p>
      `),
    }
  }

  return {
    subject: 'Email Verification OTP - KaryaSaarthi',
    html: emailWrapper(`
      <h2 style="color: #1B3A6B; margin-top: 0;">✉️ Verify Your Email</h2>
      <p style="color: #64748b;">${greeting}</p>
      <p style="color: #64748b;">Welcome to KaryaSaarthi! Please verify your email address using the OTP below:</p>
...
export function adminApprovalEmail(adminName: string) {
  return {
    subject: 'Admin Registration Request - KaryaSaarthi',
    html: emailWrapper(`
...
export function approvedEmail(name: string, department: string) {
  return {
    subject: 'Admin Access Approved - KaryaSaarthi',
    html: emailWrapper(`
...
export function rejectedEmail(name: string) {
  return {
    subject: 'Admin Registration Update - KaryaSaarthi',
    html: emailWrapper(`
...
export function projectProgressEmail({
  ...
  return {
    subject: `Project Update: "${projectName}" — KaryaSaarthi`,
    html: emailWrapper(`
...
export function newProjectMessageEmail({
  ...
  return {
    subject: `New Message in "${projectName}" — KaryaSaarthi`,
    html: emailWrapper(`
